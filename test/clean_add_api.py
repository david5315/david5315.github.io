# Clean rebuild of teacher_apis.py
with open('teacher_apis.py', 'r', encoding='utf-8') as f:
    content = f.read()

# Remove any existing public_folder_content functions
import re
content = re.sub(r'\n# =+ 公开目录.*?def public_folder_content.*?\n(?=\n@teacher_bp|\nif __name__|\Z)', '', content, flags=re.DOTALL)
content = re.sub(r'\n@teacher_bp\.route\('/public/folder-content.*?\n(?=\n@teacher_bp|\nif __name__|\Z)', '', content, flags=re.DOTALL)

# Add the API at the end
api_code = '''
