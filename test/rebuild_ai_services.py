# rebuild_ai_services.py - 重建 ai_services.py，修复语法错误

# 读取原始优化版本
with open('ai_services_optimized.py', 'r', encoding='utf-8') as f:
    content = f.read()

# 定义两套模板（使用原始优化版本的模板）
NEW_PAGE_TEMPLATE = content.split("DEFAULT_PROMPT_TEMPLATE = '''")[1].split("'''")[0]

# 创建 MODIFY_PAGE_TEMPLATE
...
""" + match.group(2)
    ...
    print('OK: Rebuilt ai_services.py with two templates')
else:
