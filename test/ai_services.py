# ai_services.py - AI服务模块，配置完全硬编码在内存中，不再读写文件
...
DEFAULT_AI_CONFIG = {
    ...
    "site_setup": {
        "quickforge_URL": "http://ai.njschool.cn",
        "public_file_for_teacher": "true",
        "quickforge_version": "quickforge"
    },
    "logged_user_key": ""  # 新增字段，用于存储登录用户的 user_key
}
#       "quickforge_version": "basic"
  
#  "quickforge_URL": "http://223.105.74.21:9992",
#    "quickforge_version": "quickforge" "school" "basic"
...
# 注意：text_model.api_key 和 image_model.api_key 不需要从环境变量读取，保持默认空值

# ==================== 硬编码的提示词模板 ====================
DEFAULT_PROMPT_TEMPLATE = '''你是一个专业的网页生成助手。请根据以下用户需求生成一个完整的、可直接运行的HTML页面（包含CSS和JavaScript），页面应能调用系统提供的API来实现所需功能。

目标：根据用户要求生成交互式网页。网页需接入统一数据中台，所有提交的数据通过中台接口存储，分析页面从中台查询数据并展示统计结果。
...
URL：/api/submit
方法：POST
支持 Content-Type：application/json 或 multipart/form-data（用于文件上传）
必填字段（所有请求均需包含）：
  - teacher：教师用户名（例如：{{teacher}} ），必须提供。
  - activity_id：活动标识（例如 course_001），必须提供。也可使用字段名 task 作为替代，但建议统一使用 activity_id。
  - student_id：学生标识，可以是学号或匿名ID。
其他自定义字段（如题目答案、操作记录等）可任意添加。

文件上传方式（如果需要）
使用 FormData，将上述必要字段作为独立的表单字段添加，其他数据可放入一个名为 data 的 JSON 字符串中。文件字段名固定为 files，可重复添加多个文件。
示例（JavaScript）：
const formData = new FormData();
formData.append('teacher', '{{teacher}} ');
formData.append('activity_id', '{{task}}');
formData.append('student_id', 'S001');
formData.append('data', JSON.stringify({ answer1: 'A', memo: '备注' }));
...
URL：/api/submissions
方法：GET
参数（通过 query string 传递）：
  - teacher（必选）：教师用户名。
  - activity_id（可选）：筛选指定活动。
  - student_id（可选）：筛选指定学生。
  - limit（可选）：返回条数限制（默认100，最大500）。
  - offset（可选）：分页偏移量（默认0）。
响应示例（重要：数据实际存储在 "submissions" 字段中）：
{
  "success": true,
  "total": 200,
  "offset": 0,
  "limit": 100,
  "submissions": [              // ← 记录数组在此字段内
    {
      "teacher": "{{teacher}} ",
      "activity_id": "'{{task}}",
      "student_id": "S001",
...
解析时，应先检查 response.ok 和 data.success，然后从 data.submissions 中获取数组。

3. 文件访问
上传的文件可通过 /templates/{{teacher}}/uploads/<filename> 直接访问。在 API 返回的 _files 字段中会包含完整的 url，可直接使用。

生成要求：
- 页面应美观、响应式，使用现代化 CSS（如 Flexbox、Grid），可引入 Tailwind CSS 或自定义样式。
- 使用原生 JavaScript（ES6+）实现交互和 API 调用，无需额外框架。
- 根据用户需求合理选择调用上述 API 接口，并处理成功/失败提示。
- **获取教师和活动标识**：页面需要当前教师的用户名（teacher）和活动标识（activity_id/task）。建议通过解析当前页面的 URL 路径动态获取（因为页面可能以静态方式部署，如直接访问 `/templates/教师名/.../活动名/页面.html`）。示例代码：
  ```javascript
  const pathParts = window.location.pathname.split('/');
  // 教师名通常在路径的第二段（索引2），需解码URL编码的中文
  const teacher = decodeURIComponent(pathParts[2]);
  // 活动名通常为最后一级文件夹名（倒数第二段）
  const activityId = decodeURIComponent(pathParts[pathParts.length - 2]);
...
def load_ai_config():
    ...
    # 注意：深拷贝，避免后续修改影响默认值
    import copy
    _ai_config = copy.deepcopy(config)
    print("✅ 使用硬编码的 AI 配置")
    return _ai_config
...
def check_ai_quota():
    """
    检查AI使用配额，返回 (can_use, message, api_key, using_master)
    现在 always using_master=False，强制使用 temp_key 并检查剩余次数。
    """
    config = get_ai_config()
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if image_url:
                ...
                print(f"⚠️ 未找到图片URL，响应: {response_data}")
                return None
        else:
            error_msg = f"图片生成API请求失败: 状态码 {response.status_code}"
            print(f"❌ {error_msg}")
            print(f"响应内容: {response.text}")
            return None
    except Exception as e:
        error_msg = f"调用图片生成API失败: {str(e)}"
        print(f"❌ {error_msg}")
        return None
...
def generate_page_from_folder():
    """AI根据参考文件夹或勾选的文件生成HTML页面"""
    print("\n=== 进入 /ai/generate-page-from-folder ===")
    try:
        can_use, msg, api_key, using_master = check_ai_quota()
        if not can_use:
            print(f"❌ 配额检查失败: {msg}")
                ...
                if file_full and os.path.isfile(file_full):
                    try:
                        with open(file_full, 'r', encoding='utf-8') as f:
                            content = f.read()
                        # ===== 修改点：不再截断内容 =====
                        ref_contents.append(f"--- 文件 {os.path.basename(file_full)} ---\n{content}")
                    except Exception as e:
                ...
                if os.path.isfile(fpath):
                    try:
                        with open(fpath, 'r', encoding='utf-8') as f:
                            content = f.read()
                        # ===== 修改点：文件夹内文件也不截断 =====
                        ref_contents.append(f"--- 文件 {fname} ---\n{content}")
                    except:
        ...
        print(f"📄 模板内容 (前10000字符): {repr(template[:10000])}...")
        ...
        print(f"🤖 最终发送给 AI 的提示词（前10000字符）: {combined_prompt[:10000]}...")
        ...
        if not generated_html:
            print("❌ AI 模型返回空，生成失败")
            return jsonify({'success': False, 'message': 'AI生成失败'}), 500
...
def ai_generate_page():
    ...
    try:
        ...
        print(f"📥 收到请求: prompt='{prompt[:50]}...', target_folder='{target_folder}', file_name='{file_name}'")
        print(f"📎 参考文件: {reference_files}")
        ...
        can_use, msg, api_key, using_master = check_ai_quota()
        print(f"🔑 配额检查: can_use={can_use}, msg={msg}, using_master={using_master}")
        if not can_use:
            print(f"❌ 配额检查失败: {msg}")
            return jsonify({'success': False, 'message': msg}), 403
        ...
        template = load_prompt_template()
        print(f"📄 模板内容 (前200字符): {repr(template[:200])}...")

        combined_prompt = template.replace('{{prompt}}', prompt) \
                                   .replace('{{ref_content}}', ref_content) \
                                   .replace('{{teacher}}', current_teacher) \
                                   .replace('{{task}}', target_folder)
        ...
        print(f"🤖 最终发送给 AI 的提示词（前200字符）: {combined_prompt[:200]}...")
...
def call_ai_model_for_html(prompt, model, base_url, api_key):
    """调用文本模型生成HTML，返回生成的HTML文本或None"""
    print("--- 进入 call_ai_model_for_html ---")
    try:
        payload = {
            "model": model,
            "messages": [
                {"role": "system", "content": "你是一个专业的网页生成助手，只返回纯HTML代码，不要包含任何额外解释。"},
