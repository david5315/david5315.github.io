...
# 强制使用 UTF-8 编码输出（解决 Windows 控制台 emoji 显示问题）
...
def initialize_folders():
    """
    从打包资源中恢复默认文件夹到可执行文件目录。
    对于 data/shared 目录下的 JSON 配置文件，如果目标文件不存在或为空，则从打包资源复制。
    对于其他文件，仅在目标文件不存在时复制（保留用户数据）。
    """
    exposed_folders = [
    ...
    for rel_path, target_dir in exposed_folders:
        ...
        for root, dirs, files in os.walk(src_dir):
            ...
            for file in files:
                ...
                # 对于 data/shared 目录下的 JSON 文件，如果目标文件不存在或为空，则复制
                if 'data/shared' in dest_file.replace('\\', '/') and file.endswith('.json'):
                    should_copy = False
                    ...
                    if should_copy:
                        ...
                        print(f"[OK] 从打包资源恢复配置文件：{dest_file}")
                else:
                    # 其他文件：仅在不存在时复制
                    if not os.path.exists(dest_file):
                        shutil.copy2(src_file, dest_file)
